...
interface ClientOverrideTemplate {
  id: string;
  contactId: string;
  name: string;
  items: Item[];
...
export function ChecklistClientSubTab({ checklistType }: Props) {
  const [contacts, setContacts] = useState<ContactOption[]>([]);
  const [templates, setTemplates] = useState<ClientOverrideTemplate[]>([]);
    ...
    try {
      const [contactsRes, templatesRes] = await Promise.all([
        supabase
          ...
          .in('contact_type', ['agent', 'client', 'tc', 'other'])
          .eq('is_active', true)
          .order('full_name', { ascending: true })
          .limit(200),
        supabase
          .from('checklist_templates')
          .select(`id, name, contact_id, checklist_template_items ( id, title, is_required, sort_order )`)
  ...
  const currentTemplate = templates.find(t => t.contactId === selectedContactId);
  ...
  const saveItems = async (items: Item[]) => {
    ...
    try {
      ...
      const templateName = `${selectedContactName} ${checklistType === 'dd' ? 'DD' : 'Compliance'} Template`;
  ...
  const deleteTemplate = async (templateId: string) => {
    ...
    if (tpl && selectedContactId === tpl.contactId) setSelectedContactId('');
    await loadAll();
  };

  const contactsWithTemplate = new Set(templates.map(t => t.contactId));
    ...
    <div className="max-w-3xl mx-auto flex flex-col gap-5">
      ...
      <div>
        <h2 className="text-base font-bold text-black">
          Client {checklistType === 'dd' ? 'Due Diligence' : 'Compliance'} Templates
        </h2>
        <p className="text-xs text-black/50 mt-0.5">
          Add items specific to individual agents or clients — merged on top of master + MLS items.
        </p>
        <div className="mt-2 flex items-start gap-2 bg-purple-50 border border-purple-200 rounded-lg px-3 py-2">
          <AlertCircle size={13} className="text-purple-500 mt-0.5 shrink-0" />
          <p className="text-xs text-purple-700">
            Example: "Check MLS data accuracy" for a client who always wants that verified, or
            "Send thank you gift" for a specific agent preference.
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3 items-end">
        <div className="flex-1">
          <label className="label py-0.5"><span className="label-text text-xs font-medium">Select Agent / Client</span></label>
          <select
          ...
          >
            ...
            {contacts.map(c => (
              <option key={c.id} value={c.id}>
                {c.name}{contactsWithTemplate.has(c.id) ? ' ✓' : ''}
              </option>
            ))}
          </select>
        </div>
      ...
      {!selectedContactId && templates.length > 0 && (
        <div className="border border-base-300 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-base-300">
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-black/50">Contact</th>
            ...
            <tbody>
              {templates.map((tpl, idx) => {
                const contact = contacts.find(c => c.id === tpl.contactId);
                return (
                  <tr
                    key={tpl.id}
                    className={`border-b border-base-300 last:border-0 cursor-pointer hover:bg-purple-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                    onClick={() => setSelectedContactId(tpl.contactId)}
                  >
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        <User size={13} className="text-purple-500" />
                        <span className="text-xs font-medium">{contact?.name ?? tpl.contactId}</span>
                        {contact?.role && (
                          <span className="text-xs text-black/40 capitalize">({contact.role})</span>
                        )}
                      </div>
                    </td>
      ...
      {!selectedContactId && templates.length === 0 && (
        <div className="text-center py-10 text-black/30 text-xs border border-base-300 rounded-xl">
          No client templates yet. Select a contact above to create one.
        </div>
      )}

      {/* Item editor for selected contact */}
      {selectedContactId && (
        <>
          <div className="flex items-center gap-2">
            <User size={14} className="text-purple-500" />
            <span className="text-sm font-semibold text-black">{selectedContactName}</span>
                ...
                {currentItems.length === 0 && (
                  <tr>
                    <td colSpan={checklistType === 'dd' ? 5 : 4} className="text-center py-10 text-black/30 text-xs">
                      No extra items for this client yet. Add below.
                    </td>
                  </tr>
                )}
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={`Add an extra ${checklistType === 'dd' ? 'DD' : 'compliance'} item for ${selectedContactName}…`}
              value={newTitle}
      ...
      <ConfirmModal
        ...
        message="Remove this item from the client template?"
        confirmLabel="Remove"
      ...
      <ConfirmModal
        ...
        message="This will remove all custom items for this client. Master + MLS items still apply."
        confirmLabel="Delete"
