...
const extractDealSchema = {
  ...
  properties: {
    ...
    saleContingency: { type: 'string', enum: ['IS Contingent', 'NOT Contingent', 'unknown'] },
    saleType: { type: 'string', enum: ['Cash', 'Financed', ''] },
  ...
  required: [
    ...
    'transactionType', 'saleContingency', 'saleType', 'contractPrice', 'earnestMoney', 'earnestMoneyHolder', 'earnestMoneyForm', 'earnestMoneyRefundable', 'additionalEarnestMoney',
...
async function handleExtractDeal(apiKey: string, body: any) {
  ...
  // Derive form slug for DB lookups (corrections + appendix + field_coordinates)
  ...
  try {
    ...
    if (fieldRows && fieldRows.length > 0) {
      ...
      const groupOutputMap: Record<string, string> = {
        sale_type:                'saleType — return "Cash" if L296 (THIS IS A CASH SALE) is checked, "Financed" if L299 (THIS IS A FINANCED SALE) is checked',
        sale_contingency:         'saleContingency — return "IS Contingent" if L290 checkbox is checked, "NOT Contingent" if L288 checkbox is checked',
        primary_loan_type:        'loanType — return EXACTLY one of: "Conventional", "FHA", "VA", "USDA", "Other", "Owner Financing" (exact case, no lowercase) based on which row is checked in the Primary Loan column. NEVER return empty string if saleType is "Financed".',
      ...
      const detectedFormLabel = (contractDetection?.formName && contractDetection.formName !== 'Unknown')
        ? contractDetection.formName
        : 'Residential Sale Contract';
      fieldSchemaSection = `\n\nFORM FIELD REFERENCE MAP (${detectedFormLabel})
...
${keyFields}`;
  ...
  let knownCorrectionsBlock = '';
  try {
    let corrQuery = supabase
      .from('extraction_corrections')
      .select('field_key, corrected_value')
      .not('corrected_value', 'is', null)
      .not('corrected_value', 'eq', '');

    if (formSlug) {
      corrQuery = corrQuery.or(`form_slug.eq.${formSlug},form_slug.is.null`);
    }

    const { data: corrRows } = await corrQuery.limit(200);

    if (corrRows && corrRows.length > 0) {
      // Tally most common corrected value per field_key
      const tally: Record<string, Record<string, number>> = {};
      for (const row of corrRows) {
        if (!tally[row.field_key]) tally[row.field_key] = {};
        const v = row.corrected_value as string;
        tally[row.field_key][v] = (tally[row.field_key][v] || 0) + 1;
      }

      // Keep only fields with ≥2 corrections to the same value (signal, not noise)
      const hints: string[] = [];
      for (const [fieldKey, valueCounts] of Object.entries(tally)) {
        const sorted = Object.entries(valueCounts).sort((a, b) => b[1] - a[1]);
        const [topValue, topCount] = sorted[0];
        if (topCount >= 2) {
          hints.push(`• ${fieldKey}: most often corrected to "${topValue}" (${topCount}× by TC)`);
        }
      }

      if (hints.length > 0) {
        knownCorrectionsBlock = `\n\nKNOWN CORRECTION PATTERNS (from past TC reviews on this form type):\nThese fields have been frequently corrected by the TC after extraction — double-check each carefully:\n${hints.join('\n')}`;
      }
    }
  } catch (e) {
    console.warn('[extract-deal] Known corrections fetch failed (non-fatal):', e);
  }

  // ── Form prompt appendix ────────────────────────────────────────────────────
  // Form-specific extraction guidance stored in DB, keyed by form_slug.
  // Allows per-form instructions to be managed in Settings without code deploys.
  let appendixBlock = '';
  try {
    if (formSlug) {
    ...
    console.warn('[extract-deal] Form appendix fetch failed (non-fatal):', e);
  }
...
For saleContingency: Find the sale contingency checkboxes in the contract. Look for two options — one stating this contract IS contingent on the sale/closing of a buyer's property, and one stating it is NOT contingent. Return "IS Contingent" if the contingent option is checked, "NOT Contingent" if the non-contingent option is checked. Return "unknown" if neither is visible.
For saleType: Find the checkboxes indicating whether this is a cash or financed transaction. Look for options labeled "THIS IS A CASH SALE" or "THIS IS A FINANCED SALE" (or equivalent language). Exactly one will be checked. Return "Cash" if the cash option is checked, "Financed" if the financed option is checked. Return "" if not determinable.
For loanOccupancyType: find the occupancy or financing type checkboxes in the loan terms section. Look for options indicating owner-occupied or investment use. Return "owner-occupied" if owner-occupied is checked, "investment" if investment is checked, or empty string "" if not found.
...
For earnestMoneyRefundable: Find the refundability checkboxes in the Earnest Money section (look for a line labeled "(Check one) refundable / non-refundable" or equivalent language). There are exactly TWO checkboxes: the FIRST checkbox is next to the word "refundable" and the SECOND checkbox is next to "non-refundable". Look carefully at which checkbox has a mark (☑, ✓, X, or filled square). IMPORTANT: The "refundable" checkbox comes FIRST (left side). If the FIRST/LEFT checkbox is marked → return the STRING "Refundable". If the SECOND/RIGHT checkbox is marked → return the STRING "Non-refundable". Do NOT confuse "refundable" with "non-refundable" — they are separate checkboxes. Return empty string "" only if this line is completely absent. NEVER return null.
...
For buyerAgentCommission: Find the section where the seller agrees to compensate the broker assisting the buyer from seller's funds at closing. This is often labeled "Seller Compensation to Broker assisting Buyer", "SELLER Compensation to Broker assisting BUYER", or similar language in the closing costs or commission section. The dollar amount stated there is the buyer agent commission. Return as numeric string (e.g. '9420'). Do NOT put this value in sellerPaidClosingCosts. Return null if not found.
...
For warrantyArranger: find the section indicating who is responsible for arranging the home warranty plan. Return exactly "Licensee assisting SELLER" or "Licensee assisting BUYER" based on which checkbox is checked. Return null if no warranty or not specified.
...
NOTE ON TWO-COLUMN LICENSEE LAYOUT: Some contracts show two side-by-side columns — "Listing Licensee" (seller's agent) and "Selling Licensee" (buyer's agent). If your contract uses this layout, each column is self-contained. Extract the name from WITHIN each column only — never cross columns.
For mlsBoard: extract the MLS board or association name (e.g., "Heartland MLS", "KCRAR"). Return null if not found.
For loanType: Find the loan type / financing type section in the contract. Look for checkboxes indicating the primary loan type: Conventional, FHA, VA, USDA, Other, or Owner Financing. If the contract uses a Primary Loan / Secondary Loan column layout, look at the Primary Loan (left) column only. Return EXACTLY one of: "Conventional", "FHA", "VA", "USDA", "Other", "Owner Financing". CRITICAL — use EXACT capitalization shown here, never lowercase. If saleType is "Financed" you MUST return one of these six values — never return empty string. Return empty string "" ONLY if this is a cash sale. NEVER return null.
For downPaymentPercent: look for an EXPLICITLY stated down payment percentage in the financing section (e.g., "10%", "20% down"). Do NOT use the amortization period years ("30 years", "15 years" — that is a loan term, NOT a down payment). Do NOT use loan amount or LTV dollar figures. Return a percentage string (e.g., "20") ONLY if a percentage is directly written. Return null if no percentage is explicitly stated.
...
For fieldSources: output an ARRAY. For EVERY field you extract with a non-null value, push an entry: { field: 'exactFieldName', page: (PDF page number 1-indexed), line: (printed line number if visible, otherwise null), text: 'exact raw sentence from contract — verbatim, max 200 chars' }. Example: [{ field: 'buyerAgentCommission', page: 4, line: 207, text: 'SELLER agrees to pay Broker assisting BUYER from SELLER funds at Closing ... $9,420' }]. This lets the TC verify each extracted value directly against the contract.` + fieldSchemaSection + knownCorrectionsBlock + appendixBlock;
  ...
  // ── Agent name post-processing (Listing/Selling Licensee convention) ────────
