...
/**
 * Returns the task list to use for the "missing title company" auto-generation rule.
 *
 * If `titleCompanyName` is empty/falsy AND the task is not already present in
 * `existingTasks`, appends the "Enter EM Held With" task and returns the new array.
 * Otherwise returns `existingTasks` unchanged.
 *
 * @param titleCompanyName - The title company value from the deal or form.
 * @param existingTasks    - Current task array (defaults to [] for new deals).
 */
export const buildMissingTitleCompanyTasks = (
  titleCompanyName: string | undefined | null,
  existingTasks: DealTask[] = [],
): DealTask[] => {
  const alreadyHasTask = existingTasks.some(t => t.title.includes('EM Held With'));
  if (!titleCompanyName && !alreadyHasTask) {
    const task: DealTask = {
      id: generateId(),
