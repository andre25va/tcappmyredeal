...
For buyerAgentName: extract the full name of the buyer's agent/licensee. On Heartland MLS / Kansas City contracts, the signature page has TWO COLUMNS side by side — the SELLER side is on the LEFT and the BUYER side is on the RIGHT. Find the label "Name of Licensee assisting Buyer (Please Print)" in the RIGHT column — the buyer agent's name is written on the blank line IMMEDIATELY ABOVE that label (the label describes the field above it, not below). IMPORTANT: do NOT use the "BUYER" or "SELLER" signature lines — those are client/party signatures, not agent names. Return the agent's personal name only (not the brokerage/company name). Return null if not found.
For sellerAgentName: extract the full name of the seller's agent/licensee. On Heartland MLS / Kansas City contracts, the signature page has TWO COLUMNS side by side — the SELLER side is on the LEFT and the BUYER side is on the RIGHT. Find the label "Name of Licensee assisting Seller (Please Print)" in the LEFT column — the seller agent's name is written on the blank line IMMEDIATELY ABOVE that label (the label describes the field above it, not below). IMPORTANT: do NOT use the "BUYER" or "SELLER" signature lines — those are client/party signatures, not agent names. Return the agent's personal name only (not the brokerage/company name). Return null if not found.
