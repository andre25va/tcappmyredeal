...
For buyerAgentName: extract the full name of the buyer's agent/licensee. On Heartland MLS / Kansas City contracts, find the label "Name of Licensee assisting Buyer (Please Print)" — the agent's name is written on the blank line IMMEDIATELY ABOVE that label (the label describes the field above it, not below). IMPORTANT: do NOT use the "BUYER" or "SELLER" signature lines near the top of that section — those are client/party signatures, not agent names. The licensee name fields appear just above their respective labels, near the BROKERAGE and ADDRESS rows at the bottom of the signature page. Return the agent's personal name (not the brokerage/company name). Return null if not found.
For sellerAgentName: extract the full name of the seller's agent/licensee. On Heartland MLS / Kansas City contracts, find the label "Name of Licensee assisting Seller (Please Print)" — the agent's name is written on the blank line IMMEDIATELY ABOVE that label (the label describes the field above it, not below). IMPORTANT: do NOT use the "BUYER" or "SELLER" signature lines near the top of that section — those are client/party signatures, not agent names. The licensee name fields appear just above their respective labels, near the BROKERAGE and ADDRESS rows at the bottom of the signature page. Return the agent's personal name (not the brokerage/company name). Return null if not found.
