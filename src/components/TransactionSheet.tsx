...
import { X, Printer, MapPin, DollarSign, Calendar, Users, AlertTriangle, CheckSquare, FileText, StickyNote, User, Copy, Check } from 'lucide-react';
import { generatePdfId } from '../utils/pageTracking';
...
export const TransactionSheet: React.FC<Props> = ({ deal, onClose }) => {
  // Stable PDF ID for this sheet instance (stays the same across re-renders)
  const pdfId = useRef<string>(generatePdfId()).current;
  const [pdfIdCopied, setPdfIdCopied] = useState(false);

  const handleCopyPdfId = async () => {
    try {
    ...
    setPdfIdCopied(true);
    setTimeout(() => setPdfIdCopied(false), 2000);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 print:bg-white print:p-0 print:block">
      <div className="bg-white w-full max-w-3xl max-h-[92vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden print:max-h-none print:shadow-none print:rounded-none">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-gray-900 text-white flex-none print:hidden">
          ...
          <div className="flex items-center gap-2">
            {/* PDF ID — shown in header for easy copying before printing */}
            <button
              onClick={handleCopyPdfId}
              title={pdfIdCopied ? 'Copied!' : 'Copy PDF ID'}
              className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-mono"
            >
              {pdfIdCopied ? <Check size={12} className="text-green-400" /> : <Copy size={12} />}
              <span className="text-gray-300">{pdfId}</span>
            </button>
            <button
              onClick={handlePrint}
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6 print:p-4 print:space-y-4">
          ...
          <div className="text-center pt-2 pb-1 space-y-0.5">
            ...
            <p className="text-xs text-gray-400 font-mono tracking-wide">{pdfId}</p>
          </div>
        </div>
      </div>
    </div>
