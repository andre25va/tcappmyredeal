// fetch-mls-number Edge Function v11
// Architecture:
//   address provided → Realist VPS (zip, county, apn, owner) + OpenAI (legal desc)
//   mlsNumber only   → OpenAI web search (as before)
...
// ─── OpenAI legal description lookup ─────────────────────────────────────────
...
// ─── OpenAI full property search (MLS# fallback) ─────────────────────────────
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (address && address.trim()) {
      ...
      if (realistData) {
        // Got Realist data — optionally enrich with OpenAI for legal description
        let legalDescription = realistData.legal || null;
        if (!legalDescription && openAiKey && realistData.zip && realistData.county) {
          console.log('Enriching with OpenAI legal description...');
        ...
        return new Response(JSON.stringify({
          ...
          data: {
            ...
            city: realistData.city || city || null,
            state: realistData.state || state || null,
            zipCode: realistData.zip || zipCode || null,
