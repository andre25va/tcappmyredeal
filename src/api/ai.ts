...
For buyerAgentName: extract the full name of the buyer's agent (also called buyer's representative, buyer's broker, selling agent, or buyer's licensee). On Heartland MLS / Kansas City contracts, the last page has a two-column layout — the SELLER column is on the LEFT and the BUYER column is on the RIGHT. The buyer's agent name appears in the RIGHT (BUYER) column under the label "Name of Licensee assisting Buyer (Please Print)" — the printed name written in the blank above or on that label line. Also check: the commission section (section f), the broker certification block, the licensee signature area, or any line labeled "Buyer's Agent", "Buyer's Broker", "Selling Agent", "Licensee assisting Buyer", or "Selling Licensee". CRITICAL: Do NOT return the name from the "Name of Licensee assisting Seller" line — that is a different person. Return the agent's personal name (not the brokerage/company name). Return null if not found.
For sellerAgentName: extract the full name of the seller's agent (also called listing agent, seller's broker, seller's representative, or seller's licensee). On Heartland MLS / Kansas City contracts, the last page has a two-column layout — the SELLER column is on the LEFT and the BUYER column is on the RIGHT. The seller's agent name appears in the LEFT (SELLER) column under the label "Name of Licensee assisting Seller (Please Print)" — the printed name written in the blank above or on that label line. Also check: the commission section (section f), the broker certification block, the licensee signature area, or any line labeled "Listing Agent", "Seller's Agent", "Seller's Broker", "Licensee assisting Seller", or "Listing Licensee". CRITICAL: Do NOT return the name from the "Name of Licensee assisting Buyer" line — that is a different person. Return the agent's personal name (not the brokerage/company name). Return null if not found.
