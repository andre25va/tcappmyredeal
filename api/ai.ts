...
For buyerAgentName: find the exact label text "Name of Licensee assisting Buyer (Please Print)" anywhere in the document. The buyer agent's name is the handwritten or printed text that appears on the blank line IMMEDIATELY BEFORE (above) that label in the document's reading order. Do NOT use the line labeled "BUYER" — that is the buyer client's name, not the agent. Do NOT use the BROKERAGE line. IMPORTANT: Buyer and seller names often appear early in the contract (near the top) and may be followed by designations like "AMC" (Applying as Married Couple) or "ASP" (Applying as Single Person) — these are buyer/seller status codes, NOT agent indicators. A name followed by AMC or ASP is always a buyer or seller client, never an agent. Real estate agents/licensees/REALTORS are typically found near the END of the contract in the signature/licensee section. Return the agent's personal name only (not the brokerage/company name). Return null if the label is not found or the line above it is blank.
For sellerAgentName: find the exact label text "Name of Licensee assisting Seller (Please Print)" anywhere in the document. The seller agent's name is the handwritten or printed text that appears on the blank line IMMEDIATELY BEFORE (above) that label in the document's reading order. Do NOT use the line labeled "SELLER" — that is the seller client's name, not the agent. Do NOT use the BROKERAGE line. Do NOT use the buyer's name or seller's name. IMPORTANT: Buyer and seller names often appear early in the contract (near the top) and may be followed by designations like "AMC" (Applying as Married Couple) or "ASP" (Applying as Single Person) — these are client status codes, NOT agent indicators. A name followed by AMC or ASP is always a buyer or seller client, never an agent. Real estate agents/licensees/REALTORS are typically found near the END of the contract in the signature/licensee section. If the line above the label is blank or not clearly a licensee name, return null. Return the agent's personal name only (not the brokerage/company name).
