...
export const DealChatPanel: React.FC<Props> = ({ deal, onUpdate }) => {
  ...
  const logAudit = async (action: string, entityType: string, entityId: string, entityName: string, aiSuggestion: DealChatAction, approved: boolean) => {
    try {
  ...
  const handleApproveAction = async (action: DealChatAction) => {
    ...
    switch (action.type) {
      ...
      case 'add_note': {
        ...
        const updatedDeal = {
          ...deal,
          activityLog: [noteEntry, ...(deal.activityLog || [])],
        };
        onUpdate(updatedDeal);
        ...
        setMessages(prev => [...prev, {
          id: `sys-${Date.now()}`,
          role: 'assistant',
          content: `\u2705 Note added: **${noteEntry.action}**`,
          timestamp: new Date().toISOString(),
        }]);
        break;
      }
      case 'flag_compliance_issue': {
        ...
        setMessages(prev => [...prev, {
          id: `sys-${Date.now()}`,
          role: 'assistant',
          content: `\u26a0\ufe0f Compliance issue flagged: **${String(p.title || '')}** \u2014 Review in Checklists tab.`,
          timestamp: new Date().toISOString(),
        }]);
        break;
      }
      ...
      case 'suggest_stage_update': {
        ...
        setMessages(prev => [...prev, {
          id: `sys-${Date.now()}`,
          role: 'assistant',
          content: `\ud83d\udd04 Stage update noted: consider moving to **${String(p.title || '')}**. Update manually in Edit Deal for safety.`,
          timestamp: new Date().toISOString(),
        }]);
        break;
      }
