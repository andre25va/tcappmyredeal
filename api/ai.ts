...
For buyerAgentName: extract the full name of the buyer's agent/licensee. On Heartland MLS / Kansas City contracts, look ONLY for the line explicitly labeled "Name of Licensee assisting Buyer (Please Print)" — the printed name on that specific line is the buyer agent. IMPORTANT: do NOT use the "BUYER" signature line (where the actual buyer/client signs their name) — that is the client, not the agent. The licensee name fields appear in a separate row below the buyer/seller signature blocks, typically near the brokerage name and contact info. Also check: the commission section (section f) or any line labeled "Buyer's Agent", "Buyer's Broker", or "Selling Agent". Return the agent's personal name (not the brokerage/company name). Return null if not found.
For sellerAgentName: extract the full name of the seller's agent/licensee. On Heartland MLS / Kansas City contracts, look ONLY for the line explicitly labeled "Name of Licensee assisting Seller (Please Print)" — the printed name on that specific line is the seller agent. IMPORTANT: do NOT use the "SELLER" signature line (where the actual seller/client signs their name) — that is the client, not the agent. The licensee name fields appear in a separate row below the buyer/seller signature blocks, typically near the brokerage name and contact info. Also check: the commission section (section f) or any line labeled "Listing Agent", "Seller's Agent", or "Seller's Broker". Return the agent's personal name (not the brokerage/company name). Return null if not found.
