...
export const VoiceUpdatePanel: React.FC<Props> = ({ deal, onUpdate }) => {
  ...
  // ── Action Handlers (same pattern as DealChatPanel) ──────────────────────
  ...
  const logAudit = async (
    ...
    try {
      await supabase.from('audit_log').insert({
        ...
        metadata: {
          source: 'voice_update',
          aiActionType: aiSuggestion.type,
          aiPayload: aiSuggestion.payload,
  ...
  const handleApproveAction = async (action: DealChatAction) => {
    ...
    switch (action.type) {
      case 'create_task': {
        ...
        onUpdate({ ...deal, tasks: [...(deal.tasks || []), newTask] });
        await logAudit('ai_task_created', 'task', newTask.id, newTask.title, action, true);
        break;
      }
      case 'add_note': {
        const noteEntry = {
          ...
          action: String(p.title || 'Voice Note'),
          detail: String(p.description || ''),
          user: 'AI Voice',
          type: 'note' as const,
        };
        onUpdate({ ...deal, activityLog: [noteEntry, ...(deal.activityLog || [])] });
        await logAudit('ai_note_added', 'note', noteEntry.id, noteEntry.action, action, true);
        break;
      }
      case 'flag_compliance_issue': {
        await logAudit('ai_compliance_flagged', 'compliance', deal.id, String(p.title || ''), action, true);
        break;
      }
      case 'draft_email': {
        await logAudit('ai_email_drafted', 'email', deal.id, String(p.title || ''), action, true);
        break;
      }
      case 'suggest_stage_update': {
        await logAudit('ai_stage_suggestion', 'deal', deal.id, `${deal.milestone} → ${String(p.title || '')}`, action, true);
        break;
      }
    }
  };
  ...
  const handleDismissAction = async (action: DealChatAction) => {
    await logAudit('ai_action_dismissed', 'deal', deal.id, action.type, action, false);
  };
