// send-nudge Edge Function - v2
// v2: UTF-8 safe subject lines — plain text labels instead of raw emojis in subject
...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const [address, dealItems] of grouped.entries()) {
      ...
      html += `<div style="background:#fff;border:2px solid ${borderColor};border-radius:10px;margin-bottom:14px;overflow:hidden;"><div style="background:${borderColor}18;border-bottom:1px solid ${borderColor}44;padding:10px 16px;"><div style="font-size:14px;font-weight:700;color:#0f172a;">${address}</div>${closingBadge ? `<div style="font-size:12px;color:#475569;margin-top:2px;">${closingBadge}</div>` : ''}</div><div style="padding:10px 16px;">`;
      for (const item of dealItems) {
        ...
        html += `<div style="padding:6px 0;border-bottom:1px solid #f1f5f9;"><div style="font-size:13px;font-weight:600;color:#1e293b;">${item.title}${overdueText}</div></div>`;
      }
      html += `</div></div>`;
    }
