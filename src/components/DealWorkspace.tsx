...
import type { DealTask } from '../ai/types';
...
export const DealWorkspace: React.FC<Props> = ({ deal, onUpdate, onBack, contactRecords = [], users = [], emailTemplates = [], complianceTemplates = [], deals = [], onCallStarted, onArchiveDeal, onRestoreDeal, onChangeStatus, initialTab, initialRequestType, onOpenInboxConversation }) => {
  ...
  // Fetch deal emails
  const { emails: dealEmails, loading: emailsLoading, stats: emailStats, refetch: refetchDealEmails } = useDealEmails(deal);

  // Linked emails unread count for tab badge
  const [linkedEmailUnread, setLinkedEmailUnread] = React.useState(0);
  const [linkedEmailListVersion, setLinkedEmailListVersion] = React.useState(0);
  ...
  // ── VOICE1: Email thread linking + task injection ──────────────────────────
  const handleLinkEmailThread = async (dealId: string, threadId: string) => {
    await supabase.from('email_thread_links').upsert(
      { deal_id: dealId, thread_id: threadId },
      { onConflict: 'deal_id,thread_id' }
    );
    refetchDealEmails();
    setLinkedEmailListVersion(v => v + 1);
  };

  const handleEmailCommandCreateTasks = (_dealId: string, tasks: DealTask[]) => {
    ...
    onUpdate({ ...deal, tasks: [...(deal.tasks || []), ...newTasks] });
  };
        ...
        {tab === 'ai-emails' && (
          <div className="p-4">
            {emailsLoading && dealEmails.length === 0 ? (
              ...
              <EmailCommandCenter deal={dealToRecord(deal)} emails={dealEmails} onLinkThread={handleLinkEmailThread} onCreateTasks={handleEmailCommandCreateTasks} />
            )}
          </div>
        )}
